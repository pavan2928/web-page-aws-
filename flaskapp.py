...
def login():
    if request.method == 'POST':
        ...
        if user:
            return redirect(url_for('profile', username=username))  # Redirect to user details if successful
        else:
...
@app.route('/logout')
def logout():
    session.pop('username', None)  # Remove username from the session if you were using sessions
    return redirect(url_for('index'))
